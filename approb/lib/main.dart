// Codul complet modificat conform cerințelor
...
import 'package:approb/src/screens/chat_screen.dart';  // Asigură-te că această linie este prezentă
  ...
  void initState() {
    ...
    translations = {
      'ro': {
        ...
        'disclaimer': 'Aplicația este oferită „ca atare”. Nu ne asumăm responsabilitatea pentru eventuale erori, buguri sau crash-uri. Utilizarea aplicației se face pe propriul risc.',
        'theme': '🌓 Tema',
        'gdprTitle': 'GDPR',
        'gdprContent': 'Acceptați termenii și condițiile aplicației?\n\nPrin accept, sunteți de acord cu procesarea datelor anonime pentru recenzii și programări.',
      ...
      'en': {
        ...
        'disclaimer': 'The app is provided "as is". We are not responsible for any bugs, errors, or crashes. Use the app at your own risk.',
        'theme': '🌓 Theme',
        'gdprTitle': 'GDPR',
        'gdprContent': 'Do you accept the app\'s terms and conditions?\n\nBy accepting, you agree to the processing of anonymous data for reviews and appointments.',
          ...
          TextButton(
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(translations[language]!['appCloseMsg']!)),
              );
              Future.delayed(const Duration(milliseconds: 800), () {
                SystemNavigator.pop();
              });
            },
            child: Text(translations[language]!['decline']!, style: const TextStyle(color: Colors.red)),
          ),
          ElevatedButton(
            onPressed: () {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Scaffold(
        body: Stack(
          children: [
            Container(color: Color.fromRGBO(105, 105, 105, 0.85)),
            ...
            Center(
              child: SingleChildScrollView(
                ...
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    for (var key in ['appointments', 'locations', 'reviews', 'videos'])
                      Padding(
                        ...
                        child: SizedBox(
                          ...
                          child: ElevatedButton(
                            ...
                            child: Text(translations[language]![key]!, style: const TextStyle(fontSize: 16)),
                          ),
                        ),
                      ),
                    ...
                    Row(
                      ...
                      children: [
                        {'icon': FontAwesomeIcons.instagram, 'color': Colors.pink, 'url': 'https://www.instagram.com/robert_frizerul'},
                        {'icon': FontAwesomeIcons.facebook, 'color': Colors.blue, 'url': 'https://www.facebook.com/stoian.robert.547'},
                        {'icon': FontAwesomeIcons.youtube, 'color': Colors.red, 'url': 'https://www.youtube.com/@Robert_Stoian'},
                        {'icon': FontAwesomeIcons.tiktok, 'color': Colors.white, 'url': 'https://www.tiktok.com/@robert_stoian'},
                      ].map((data) => Tooltip(
                        message: data['url'] as String,
                        child: GestureDetector(
                          onTapDown: (_) => _openUrl(data['url'] as String),
                          child: IconButton(
                            icon: FaIcon(data['icon'] as IconData, color: data['color'] as Color),
                            onPressed: () => _openUrl(data['url'] as String),
                          ),
                        ),
                      )).toList(),
                    ),
